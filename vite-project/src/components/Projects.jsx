      ...
      <div className="max-w-6xl mx-auto">
        ...
        {projects.filter((p) => p.featured).map((project, fi) => (
          <motion.div
            ...
            className="mb-8 rounded-3xl p-8 md:p-10 bg-card border border-border"
          >
            <div className="flex items-start gap-2 mb-2">
              <span className="inline-flex items-center gap-1 text-xs font-medium px-3 py-1 rounded-full bg-muted text-muted-foreground border border-border">
                ⭐ Featured Project
              </span>
              <span className="inline-flex items-center gap-1 text-xs font-medium px-3 py-1 rounded-full bg-muted text-muted-foreground border border-border">
                ● Live
            ...
            <div className="grid md:grid-cols-2 gap-6 mt-4">
              <div>
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <h3 className="text-2xl font-display font-bold text-foreground">{project.title}</h3>
                </div>
                <p className="text-muted-foreground leading-relaxed">{project.desc}</p>
                <div className="flex flex-wrap gap-2 mt-4">
                  {project.tech.map((t) => (
                    <span key={t} className="text-xs px-3 py-1.5 rounded-full bg-muted border border-border text-foreground">
                      {t}
                    </span>
                  ))}
                </div>
                <a
                  ...
                  className="inline-flex items-center gap-2 mt-6 px-5 py-2.5 rounded-full border border-border text-foreground text-sm hover:bg-muted transition-all duration-300"
                >
              ...
              <div className="flex items-center justify-center">
                <div className="w-full max-w-xs aspect-video rounded-xl bg-muted border border-border flex items-center justify-center">
        ...
        <div className="grid md:grid-cols-3 sm:grid-cols-2 gap-6">
          {projects.filter((p) => !p.featured).map((project, i) => (
            <motion.div
              ...
              className="flex flex-col rounded-2xl p-8 transition-all duration-300 hover:-translate-y-2 bg-card border border-border hover:border-foreground/30"
            >
              <div className="flex items-center gap-3 mb-3">
                <span className="text-2xl">{project.icon}</span>
                <h3 className="text-xl font-display font-bold text-foreground">{project.title}</h3>
              </div>
              <p className="text-muted-foreground text-sm flex-1 mt-1">{project.desc}</p>
              <div className="flex flex-wrap gap-2 mt-4">
                {project.tech.map((t) => (
                  <span key={t} className="text-xs px-2.5 py-1 rounded-full bg-muted border border-border text-foreground">
                    {t}
                  </span>
                ))}
              </div>
              {project.github !== '#' && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 mt-6 text-sm text-foreground hover:opacity-70 transition-colors"
                >
                  <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg>
                  GitHub
                </a>
